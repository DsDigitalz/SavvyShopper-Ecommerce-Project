import React from "react";

export default function OurProducts() {
  return (
    <section className="ourProducts bg-zinc-100 flex flex-col gap-[5px] px-20">
      <div className="flex items-center text-red-500 mt-20 gap-[10px]  font-semibold">
        <div className="bg-red-500 w-[20px] h-[40px] rounded-[3px]"></div>
        <div>
          <p>Our Products</p>
        </div>
      </div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[36px] font-semibold">Explore Our Products</h1>
        </div>
        <div className="bg-red-500 rounded-[5px] w-[159px] h-[56px] text-white flex justify-center">
          <button>View All</button>
        </div>
      </div>
      <div className="bg-white w-full p-[300px] mt-10"> </div>
    </section>
  );
}
